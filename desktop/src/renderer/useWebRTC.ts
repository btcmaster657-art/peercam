...
// Maximum resolution cap — frames are never upscaled beyond the source.
...
// ── Frame size normaliser ─────────────────────────────────────────────────────
// Preserves the source's native aspect ratio exactly.
// Only scales DOWN if the source exceeds the max cap — never upscales.
...
function normalizeFrameSize(sourceWidth: number, sourceHeight: number): { width: number; height: number } {
  ...
  // Floor to even — &~1 clears the lowest bit
  const width  = Math.max(2, Math.floor(sourceWidth  * scale) & ~1)
  const height = Math.max(2, Math.floor(sourceHeight * scale) & ~1)
...
export function useWebRTC() {
  ...
  // app is completely invisible — but the frame pump keeps running.
  ...
  // ── Frame pump (requester only) ───────────────────────────────────────────
  const stopFramePipe = useCallback(() => {
    // Always attempt teardown — don't short-circuit based on current state
  ...
  const startFramePipe = useCallback((stream: MediaStream) => {
    ...
    const canvas = document.createElement('canvas')
    // willReadFrequently = true avoids a GPU→CPU readback stall on every getImageData
    const ctx = canvas.getContext('2d', { willReadFrequently: true })!
    canvasRef.current = canvas
    ...
    const pump = () => {
      ...
      if (width !== lastW || height !== lastH) {
        ...
        canvas.width  = lastW
        canvas.height = lastH
        log('INFO', `frame pipe resolution: source=${v.videoWidth}×${v.videoHeight} → output=${lastW}×${lastH} ar=${(lastW/lastH).toFixed(3)}`)
      }

      if (!lastW || !lastH) return

      // Draw source into output canvas — browser scales/letterboxes automatically
      ctx.drawImage(v, 0, 0, lastW, lastH)
      if (!vcamActiveRef.current) return
  ...
  function buildPeer(ws: WebSocket, sessionId: string, initiator: boolean, stream?: MediaStream): Instance {
    ...
    peer.on('stream', (remoteStream: MediaStream) => {
      ...
      if (paramsRef.current?.role === 'requester') {
        updateStatus('connected')
        startFramePipe(remoteStream)
        // Session is live — optionally hide the tray icon
